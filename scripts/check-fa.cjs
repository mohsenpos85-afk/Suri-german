/*
 * Farsi ("fa") coverage scanner.
 * Goal: guarantee that a Farsi user NEVER sees a Kurdish/other fallback.
 * Because the render helpers fall back to the ku/base field when the uk field
 * is missing (tC → item.fa || item.ku, tEx → item.exfa || item.exku,
 * tF → item[field+'Uk'] || item[field]), every translatable record must carry
 * its uk / exfa / <field>Uk value.
 *
...
(async () => {
  ...
  Object.keys(gmod.GRAMMAR).forEach((lv) => gmod.GRAMMAR[lv].forEach((t) => {
    ['fa', 'expFa'].forEach((f) => { if (!t[f]) report('GRAMMAR topic', lv + ' "' + t.de + '" missing ' + f); });
  ...
  Object.keys(gmod.GTABLES).forEach((k) => {
    ['headersFa', 'rowsFa'].forEach((f) => { if (!gmod.GTABLES[k][f]) report('GTABLES', '"' + k + '" missing ' + f); });
  });

  // ---- in-file App.jsx blocks ----
  const MASALS = extractLiteral('const MASALS = {');
  Object.keys(MASALS).forEach((lv) => MASALS[lv].forEach((s) => {
    if (!s.fa) report('MASALS title', s.id);
    s.paragraphs.forEach((p, i) => { if (!p.fa) report('MASALS para', s.id + ' [' + i + ']'); });
  }));

  const bookDict = extractLiteral('const BOOK_DICT = {');
  Object.entries(bookDict).forEach(([k, e]) => {
    if (!e.fa) report('BOOK_DICT', k);
    if (e.ex && !e.ex_fa) report('BOOK_DICT.ex', k);
  });
  ...
  Object.entries(storyDict).forEach(([k, e]) => {
    ...
    if (!e.fa) report('STORY_DICT', k);
    if (e.ex && !e.ex_fa) report('STORY_DICT.ex', k);
  });
